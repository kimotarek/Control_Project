...
public class Graph {
    ...
    private ArrayList<Integer> Path_gains = new ArrayList<>();

    private ArrayList<String> deltas = new ArrayList<>();
    ...
    public ArrayList<ArrayList<String>> solve(int s, int e)
    {
        findCycles();
        findNontouching();
        findCombinations();
        ...
        ArrayList<ArrayList<String>> aykalam = new ArrayList<>();
        ArrayList<String> buffer=new ArrayList<>();
        for (StringBuilder forwardPath : forward_paths) {
            buffer.add(String.valueOf(forwardPath));
        }
        aykalam.add(buffer);
        buffer = new ArrayList<>();
        for (Integer pathGain : Path_gains) {
            buffer.add(pathGain + "");
        }
        aykalam.add(buffer);
        buffer = new ArrayList<>();
        for (int i = 0; i < arrayList.size(); i++) {
            ...
            for (int j = 0; j < arrayList.get(i).length(); j++) {
                char x = (char) (arrayList.get(i).charAt(j) - 1);
                y += x;
            }
            buffer.add(y);
        }
        ...
        for(int i = 0; i < nonTouchingC.size(); i++)
        {
            ...
            {
                in += nonTouchingC.get(i).get(j)+" ";
            }
            buffer.add(in);
        }
        aykalam.add(buffer);
        buffer = new ArrayList<>(deltas);
        aykalam.add(buffer);
        buffer = new ArrayList<>();
        buffer.add(delta+"");
        aykalam.add(buffer);
